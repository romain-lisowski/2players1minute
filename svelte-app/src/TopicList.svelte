...
{:else}
  <ul class="grid sm:grid-col-1 md:grid-cols-3 gap-6 max-w-7xl mx-auto mb-10 sm:mb-12 md:mb-16">
        ...
        <Link to={'/' + topic.category.id + '/' + topic.category.name.toLowerCase() + '/' + topic.id + '/' + topic.slug}>
          ...
          <span class="absolute top-4 right-4">
            <span class="flex items-center justify-end gap-2">
              ...
              <span class=" w-14 h-9 text-xs sm:text-sm md:text-xl text-center bg-gray-100 text-gray-800 rounded-sm shadow-lg">
          ...
          <span class="absolute bottom-16 md:bottom-20 left-2">
            <ul class="flex justify-end gap-2 pl-3">
              {#each topic.platforms as platform}
                <li class="px-2 py-1 text-xs sm:text-sm text-gray-100 align-middle rounded-sm shadow-lg {platform.classes}">{platform.name}</li>
              {/each}
            </ul>  
          </span>
